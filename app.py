...
def init_session_state():
    ...
    if 'tasks_sort' not in st.session_state:
        st.session_state.tasks_sort = "priority"
...
def apply_theme_css():
    ...
    * {{
    ...
    .task-priority-high::before {{
        background: {theme['error']} !important; /* Vermelho (Alta) */
    }}
    
    .task-priority-medium::before {{
        background: {theme['warning']} !important; /* Amarelo (Média) */
    }}
    
    .task-priority-low::before {{
        background: {theme['success']} !important; /* Verde (Baixa) */
    }}
...
def task_form():
    ...
    priority_map_pt = {"low": "Baixa", "medium": "Média", "high": "Alta"}
    priority_map_en = {"Baixa": "low", "Média": "medium", "Alta": "high"}
    ...
    with st.form("task_form_main", clear_on_submit=True):
        ...
        with col3:
            priority_options_pt = list(priority_map_pt.values())
            current_priority_pt = priority_map_pt.get(task_data.get("priority", "medium"), "Média")
            
            priority_pt = st.selectbox(
                "Prioridade",
                options=priority_options_pt,
                index=priority_options_pt.index(current_priority_pt)
            )
            # Converte de volta para inglês para salvar no banco
            priority = priority_map_en[priority_pt]
...
def dashboard_screen():
    ...
    # Inicializa o filtro com o valor atual do session state
    current_filter_pt = priority_map_pt.get(st.session_state.tasks_filter, "Todas")
    if st.session_state.tasks_filter == "all":
        current_filter_pt = "Todas"
        
    selected_priority_pt = col_filter1.selectbox(
        "Filtrar por Prioridade",
        options=priority_options_pt,
        index=priority_options_pt.index(current_filter_pt),
        key="filter_priority_select"
    )
    st.session_state.tasks_filter = priority_map_en.get(selected_priority_pt, "all")
    
    # 2. Ordenar por
    sort_options = {"priority": "Prioridade", "created_at": "Criação (Mais Antiga)", "due_date": "Data Limite"}
    ...
    # 3. Pesquisar
    st.session_state.tasks_search = col_filter3.text_input("Pesquisar por Título/Descrição", value=st.session_state.tasks_search, key="search_input")
    
    # --- Lógica de Filtro e Ordenação ---
    # Usamos get_tasks para pegar todas as tarefas, incluindo as concluídas
    all_tasks = get_tasks(user["id"])
    
    # 1. Lógica de "Tarefa some quando conclui" (apenas tarefas pendentes)
    filtered_tasks = [task for task in all_tasks if not task.get("completed", False)]
    
    # 2. Filtrar por Prioridade
    if st.session_state.tasks_filter != "all":
        priority_en = st.session_state.tasks_filter
        filtered_tasks = [task for task in filtered_tasks if task.get("priority", "medium") == priority_en]
        
    # 3. Filtrar por Pesquisa
    if st.session_state.tasks_search:
        search_term = st.session_state.tasks_search.lower()
        filtered_tasks = [
    ...
    if st.session_state.tasks_sort == "priority":
        # Ordena por Prioridade (Alta primeiro) e Data Limite (mais próxima primeiro)
        # reverse=True para prioridade (Alta = 3, Média = 2, Baixa = 1)
        sorted_tasks = sorted(filtered_tasks, key=lambda x: (priority_order.get(x.get("priority", "medium"), 0), x.get("due_date", "9999-12-31")), reverse=True)
    elif st.session_state.tasks_sort == "due_date":
        # Ordena por Data Limite (mais próxima primeiro)
        sorted_tasks = sorted(filtered_tasks, key=lambda x: x.get("due_date", "9999-12-31"), reverse=False)
    elif st.session_state.tasks_sort == "created_at":
        # Ordena por Data de Criação (mais antiga primeiro)
        sorted_tasks = sorted(filtered_tasks, key=lambda x: x.get("created_at", "0000-01-01"), reverse=False)
    else:
...
def pending_tasks_screen():
    # A tela de "Tarefas Pendentes" no menu lateral agora será apenas um atalho para a dashboard,
    # já que a dashboard principal agora só mostra tarefas pendentes e tem os filtros.
    st.session_state.current_screen = "dashboard"
    st.rerun()
